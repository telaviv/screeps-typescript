...
export const addEnergyTask = wrap(
    (creep: ResourceCreep, opts: AddEnergyTaskOpts = {}): boolean => {
        const taskMap = [
        ...
        for (const { tasker, name } of taskMap) {
            ...
            if (ret) {
                if (creep.memory.tasks.length === 0) {
                    Logger.error(
                        'addEnergyTask:failureToMakeTask',
                        name,
                        creep.name,
                    )
                    return false
        ...
        Logger.debug(
            'addEnergyTask:failure',
            'no tasks could be made',
            creep.name,
        )
        return false
    },
