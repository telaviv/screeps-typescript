        ...
        for (const room of this.rooms) {
            ...
            if (warDepartment.status === WarStatus.SPAWN &&
                ...
                ((warDepartment.targetRoom?.controller &&
                    !warDepartment.targetRoom?.controller.my) || warDepartment.hasHostiles())) {
    ...
    private findSaviors(): void {
        for (const room of this.rooms) {
            if (room.controller && room.controller.my && hasNoSpawns(room)) {
                ...
                if (savior) {
                    savior.memory.war = { status: WarStatus.SPAWN, target: room.name, type: 'savior' } as SpawnWarMemory
                }
            }
        }
    }
